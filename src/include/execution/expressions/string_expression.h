 ...
 public:
  ...
  auto Compute(const std::string &val) const -> std::string {
    ...
    if (expr_type_ == StringExpressionType::Lower) {
      std::transform(val.cbegin(), val.cend(), ans.begin(), [](unsigned char c) { return std::tolower(c); });
    } else if (expr_type_ == StringExpressionType::Upper) {
      std::transform(val.cbegin(), val.cend(), ans.begin(), [](unsigned char c) { return std::toupper(c); });
    }
    return ans;
  }
